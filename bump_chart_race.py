...
Generates race.html which loads pre‑computed ranking data from JSON files
at runtime. Fully client‑side — works on GitHub Pages. Run build_data.py
first to populate the data/ directory.
"""
...
function buildLegend() {
  // Clear existing legend items (keep the badge)
...
function loadSeason(code) {
  document.getElementById('chart').innerHTML = '<div class="loading">Loading ' + code + ' season\u2026</div>';
  if (seasonCache[code]) {
    setData(seasonCache[code]);
    return;
  }
  fetch('data/season-' + code + '.json')
    .then(function(r) { return r.json(); })
    .then(function(json) {
      seasonCache[code] = json;
      setData(json);
    })
    .catch(function(e) {
      document.getElementById('chart').innerHTML = '<div class="loading" style="color:#dc2626">Failed to load season data.</div>';
      console.error(e);
    });
}
...
fetch('data/seasons.json')
  ...
  .catch(function() {
    ...
    seasonSelect.innerHTML = '<option value="2526">2025–26</option>';
    loadSeason('2526');
  });
