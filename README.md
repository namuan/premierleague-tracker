...
This writes `race.html`. The Python scripts only need `pandas` (auto-installed by `uv`).

### 3. Deploy
Push `race.html` and the `data/` directory. Enable GitHub Pages on the repo — no build step needed.
...
├── race.html             # The animated bump chart (client-side)
├── build_data.py         # Script to download & compute season data
├── bump_chart_race.py    # Script to generate race.html
└── README.md
```
